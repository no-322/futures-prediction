...
# Backtest Explorer: the top-5 leaderboard models and their saved single-split
# order-flow / HMM-regime variants (generated by run_stats.build_leaderboard_variants
# → `data/processed/ss_*_predictions.npz`). Each tuple is (base prediction stem,
# display name, family). "family" decides which variants exist: linear (logistic)
# models got HMM-only variants; non-linear (tree) models got order-flow + HMM variants.
_TOP5_VARIANTS: list[tuple[str, str, str]] = [
    ("exp_noflat_baseline", "Logistic Regression (no-flat, v1)", "linear"),
    ("tuned_v1_baseline",   "Logistic Regression (tuned, v1)",   "linear"),
    ("tuned_v3_gbm",        "Gradient Boosting (tuned, v3)",      "nonlinear"),
    ("tuned_v3_rf",         "Random Forest (tuned, v3)",          "nonlinear"),
    ("tuned_v2_gbm",        "Gradient Boosting (tuned, v2)",      "nonlinear"),
]
# Variant label → prediction-stem prefix ("" = the base recipe itself).
_VARIANTS_BY_FAMILY: dict[str, dict[str, str]] = {
    "linear": {
        "Base (no regime)":         "",
        "+ HMM regime feature":     "ss_hmmfeat_",
        "+ HMM gate (high-vol)":    "ss_hmmgate_",
    },
    "nonlinear": {
        "Base":                          "",
        "+ order-flow + regime feature": "ss_offeat_",
...
def _test_split_arrays(
    ...
    """Per-bar returns and timestamps for the 50/50 test split (Backtest Explorer).

    Rebuilds the same contiguous second-50% test slice the single-split prediction
    sets were generated from (``df.iloc[4:]`` → time-ordered split), so the saved
    ``ss_*`` / base predictions align 1-to-1 with these arrays. ``_mtime`` is part of
    the cache key only (busts the cache when the data file changes).

    Returns:
        (bar_returns, timestamps) for the test split — bar_returns = (Close-Open)/Open.
    """
...
tab_train, tab_predict, tab_backtest = st.tabs(
    ["Training", "Prediction", "Backtest Explorer"])
...
with tab_backtest:
    st.subheader("Backtest Explorer — top-5 models with HMM / order-flow")
    st.markdown(
        "Browse the saved **single-split (50/50) test** predictions for the top-5 "
        "leaderboard models and their order-flow / causal-HMM-regime variants, and view "
        "classification stats plus a backtest (equity curve vs passive). Read-only — no "
        "retraining. Generate the variants with "
        "`python -m src.run_stats --sections lbvar` if any are missing."
    )

    import src.statistics as _stats  # noqa: E402

    bc1, bc2 = st.columns(2)
    base_display = bc1.selectbox(
        "Model", [d for _s, d, _f in _TOP5_VARIANTS], key="bt_model")
    base_stem, family = next(
        (s, f) for s, d, f in _TOP5_VARIANTS if d == base_display)
    variants = _VARIANTS_BY_FAMILY[family]
    variant_label = bc2.radio("Variant", list(variants), key="bt_variant")
    stem = f"{variants[variant_label]}{base_stem}"
    display = f"{base_display} {variant_label}"

    txn_cost = st.number_input(
        "Transaction cost per bar (0 = frictionless, 0.0001 = 1 bp)",
        min_value=0.0, value=0.0, step=0.0001, format="%.4f", key="bt_cost",
    )

    npz_path = PROC_DIR / f"{stem}_predictions.npz"
    if not npz_path.exists():
        st.warning(
            f"No saved predictions for **{display}** (`{npz_path.name}`). "
            "Generate them with `python -m src.run_stats --sections lbvar`."
        )
    else:
        try:
            d = np.load(npz_path)
            y_pred = np.asarray(d["y_pred"])
            y_true = np.asarray(d["y_true"]) if "y_true" in d else None
            gate = np.asarray(d["gate"]).astype(bool) if "gate" in d else None

            data_path = str(cfg["data"]["path"])
            train_size = float(cfg["data"].get("train_size", 0.5))
            bar_returns, timestamps = _test_split_arrays(
                data_path, train_size, Path(data_path).stat().st_mtime)

            if len(bar_returns) != len(y_pred):
                st.error(
                    f"Length mismatch: predictions have {len(y_pred):,} rows but the "
                    f"configured data file's test split has {len(bar_returns):,}. The "
                    f"`ss_*` sets must be regenerated against `{data_path}` "
                    "(`python -m src.run_stats --sections lbvar`)."
                )
                st.stop()

            # Gate variants: trade only the high-vol (gated) bars; cash otherwise.
            if gate is not None:
                cov = float(gate.mean())
                st.caption(
                    f"**HMM gate:** trading only the {int(gate.sum()):,} high-vol bars "
                    f"of {gate.size:,} ({cov * 100:.1f}% coverage); the rest are skipped."
                )
                sel = gate
            else:
                sel = np.ones(len(y_pred), dtype=bool)

            yp = y_pred[sel]
            yt = y_true[sel] if y_true is not None else None
            br = bar_returns[sel]
            ts = timestamps[sel]
            ...
            nft = st.checkbox(
                "Evaluate on no-flat bars only (drop Open==Close)",
                value=False, key="bt_nft", disabled=not has_truth,
            )
            if nft and has_truth:
                keep = br != 0
                st.caption(f"No-flat slice: kept {int(keep.sum()):,} of {keep.size:,} "
                           f"bars ({int((~keep).sum()):,} flat dropped).")
                _show_eval(yt[keep], yp[keep])
            else:
                _show_eval(yt, yp)

            st.markdown("### Backtest — $1,000, reinvested each bar")
            bt = _stats.backtest(
                yp, br, timestamps=ts, transaction_cost=float(txn_cost),
                name=display,
            )
            b = st.columns(4)
            b[0].metric("Final equity", f"${bt['final_equity']:,.2f}")
            ...
            plot_path = Path("docs/notes") / f"backtest_gui_{stem}.png"
            _stats.plot_equity_curve(bt, plot_path)
            st.image(str(plot_path),
                     caption="Equity curve — strategy vs passive buy & hold")

            report = f"# Backtest Report — {display}\n\n"
            if has_truth:
                report += _stats.format_markdown(
                    _stats.compute(yt, yp, name=display)) + "\n\n---\n\n"
            report += _stats.format_backtest_markdown(bt)
            st.download_button(
                "Download report (.md)", data=report,
                file_name=f"backtest_{stem}.md", mime="text/markdown",
            )
        except Exception as exc:  # noqa: BLE001
